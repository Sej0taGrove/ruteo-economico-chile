...
def load_combustibles_data():
    ...
    try:
        ...
        if not json_path:
            raise FileNotFoundError(f"No se encontró 'combustible_metadata_*.json' en '{SCRIPT_DIR}'.")
        ...
        for estacion in data.get('estaciones', []):
            ...
            try:
                cur.execute(
                    """
                    INSERT INTO estaciones_servicio (id_estacion_cne, nombre, marca, direccion, comuna, region, horario, ubicacion)
